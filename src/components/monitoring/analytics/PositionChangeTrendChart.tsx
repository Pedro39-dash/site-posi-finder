...
import { KeywordMetricsService, PageMetrics } from '@/services/keywordMetricsService';
import { TopPagesCompact } from './TopPagesCompact';
...
export const PositionChangeTrendChart = ({ projectId, isLoading = false }: PositionChangeTrendChartProps) => {
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        ...
        const [trendDataResult, pagesResult] = await Promise.all([
          MonitoringAnalyticsService.getTrendData(projectId, days),
          KeywordMetricsService.getPageMetrics(projectId)
        ]);
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
      ...
      {/* Coluna da direita: URLs mais acessadas */}
      <div className="lg:col-span-1">
        <TopPagesCompact data={topPages} isLoading={loading} />
      </div>
    </div>
