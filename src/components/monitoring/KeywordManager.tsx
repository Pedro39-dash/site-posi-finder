...
export const KeywordManager = ({
  ...
  useEffect(() => {
    setKeywordList(rankings.map(r => r.keyword));
  }, [rankings]);
  ...
  useEffect(() => {
    const calculateRelevance = async () => {
      ...
      const relevance = await calculateKeywordRelevance(projectId, keywords);
  ...
  // Separar keywords ativas e inativas
  const activeRankings = useMemo(() => {
    return enrichedRankings.filter(r => !r.tracking_status || r.tracking_status === 'active');
  }, [enrichedRankings]);
  ...
  const displayedActiveRankings = useMemo(() => {
    ...
    return activeRankings.filter(r => keywordSet.has(r.keyword));
  }, [activeRankings, keywordList]);
  ...
  const handleDeleteKeyword = async (keywordId: string, keyword: string) => {
    if (!confirm(`Tem certeza que deseja remover "${keyword}" do monitoramento?`)) return;
    try {
      const result = await RankingService.deleteKeyword(keywordId);
      if (result.success) {
        toast({ title: "Keyword Removida", description: `"${keyword}" foi removida do monitoramento` });
        onRankingsUpdate();
      } else {
        throw new Error(result.error);
      }
