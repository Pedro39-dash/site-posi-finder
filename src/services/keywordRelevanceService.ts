...
/**
 * Calcula a relevância de keywords baseada só em dados de hoje
 */
export async function calculateKeywordRelevance(
  projectId: string,
  keywords: string[]
  ...
  try {
    // Buscar histórico só do período atual (hoje!)
    const result = await fetchRankingHistory(projectId, keywords);
    ...
    for (const keyword of keywords) {
        ...
        try {
          ...
          if (currentRanking?.current_position) {
            dataPoints = [{
              ...
              position: currentRanking.current_position,
              change: 0,
              metadata: { is_current_only: true }
            }];
          }
        } catch { }
      }

      // Se ainda não tem dados, marca como não relevante
      if (dataPoints.length === 0) {
        relevanceMap.set(keyword, {
          keyword,
          totalDataPoints: 0,
      ...
      const firstDate = new Date(dataPoints[0].date).toISOString();
      const lastDate = new Date(dataPoints[dataPoints.length - 1].date).toISOString();
      ...
      relevanceMap.set(keyword, {
        ...
        isRelevant: true,
        relevanceReason: 'current_period',
        dataPointsInPeriod: dataPoints.length
      });
    }
...
/**
 * Sempre retorna todas as keywords sem filtragem por relevância/tempo
 */
export function filterRelevantKeywords(
  keywords: string[],
  relevanceMap: Map<string, KeywordRelevance>,
